...
| `현재상태.md` | **어디까지 됐고 다음에 뭘 하는지. 여기부터 읽으면 된다** |

(아티팩트 문서 링크는 팀 채널 참고)

## 현재 상태

**데이터 파이프라인 완료.** 특보 4단계가 모두 실제로 경로를 바꾼다.

| 단계 | 상태 |
|---|---|
| 0 환경 · 1 스키마 · 2 보행 그래프 | 완료 |
| 3 침수구역 적재 | 완료 — 3종 34개 폴리곤 |
...
의정부역 → 회룡역, 위험 판정 구간 통과 거리:

```
평상시     2,377m      0m
호우주의보 3,107m      0m
호우경보   3,151m      0m
극한호우   3,150m    426m   ← 우회로가 없는 구간
```
...
```powershell
git clone https://github.com/qkrtndus5865-web/manhole-flood-nav.git
cd manhole-flood-nav
```
